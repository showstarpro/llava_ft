...
from transformers import CLIPVisionModel, CLIPImageProcessor, CLIPVisionConfig, CLIPConfig, CLIPPreTrainedModel
import transformers
...
class DiTEncoder(nn.Module):
    ...
    def __init__(self, config: CLIPConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([DiTEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class DiTVisionTransformer(nn.Module):
    def __init__(self, config: CLIPVisionConfig):
        super().__init__()
        self.config = config
...
class DiTVisionModel(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPVisionConfig):
        ...
        self.vision_model = DiTVisionTransformer(config)
...
# txts = ['I love rice', 'I want to sleep']
# txts_token = vicuna_tokenizer(txts)['input_ids']

# decoded_texts = [vicuna_tokenizer.decode(token_ids, skip_special_tokens=True) for token_ids in txts_token]

# txts_clip_token = clip_tokenizer(decoded_texts)['input_ids']
# txts_clip_token = [torch.tensor(element) for element in txts_clip_token]

# txts_clip_token = torch.nn.utils.rnn.pad_sequence(
#             txts_clip_token,
#             batch_first=True,
#             padding_value=clip_tokenizer.pad_token_id)

# # txts_clip_token = torch.tensor(txts_clip_token)

# from transformers import CLIPTextModel
# text_tower = CLIPTextModel.from_pretrained('/lpai/volumes/so-volume-ga/models/clip-vit-large-patch14-336')
# text_outputs = text_tower(txts_clip_token)

# print(dit_model)
# import torch
# imgs = torch.randn(2, 3, 336, 336)
# c = torch.randn(2, 1024)
# outputs = dit_model(imgs, c, output_hidden_states=True)

# image_features = outputs.hidden_states[-2]



# zero_model = zero_module(conv_nd(2, 1024, 1024, 1, padding=0))
# image_features = image_features[:, 1:, :]
# B, L, D = image_features.shape
# import math
# image_features = image_features.reshape(B, int(math.sqrt(L)), int(math.sqrt(L)), D)
# image_features = image_features.permute(0, 3, 1, 2)
# zero_outputs = zero_model(image_features)
# zero_outputs= zero_outputs.permute(0, 2, 3, 1)
# zero_outputs = zero_outputs.reshape(B, -1, D)
# print(zero_outputs)
