...
from transformers.models.clip.modeling_clip import CLIPEncoderLayer, CLIPAttention, CLIPMLP, CLIPVisionEmbeddings, CLIPPreTrainedModel
import torch
...
class MMCLIPAttention(CLIPAttention):
    def __init__(self, config):
        ...
        self.instruction_gate = nn.Parameter(torch.Tensor([0.]))
        self.instruction_out_proj, self.instruct_q_proj, self.instruct_v_proj, self.instruct_k_proj = (None,) * 4
    ...
    def __init__(self, config: CLIPConfig):
        ...
        for layer_id in range(config.num_hidden_layers):
            if config.integration_point == 'all':
                ...
                layer = OurCLIPEncoderLayer if layer_id < (config.num_hidden_layers // 2) else CLIPEncoderLayer
            elif config.integration_point == 'late':
                layer = CLIPEncoderLayer if layer_id < (config.num_hidden_layers // 2) else OurCLIPEncoderLayer
            elif config.integration_point == 'late2':
                layer = CLIPEncoderLayer if layer_id < (3 * config.num_hidden_layers // 4) else OurCLIPEncoderLayer
            elif config.integration_point == 'late3':
                layer = CLIPEncoderLayer if layer_id < (config.num_hidden_layers // 4) else OurCLIPEncoderLayer
    ...
    def init_qavit_comps(self):
        with torch.no_grad():
            for layer in range(len(self.base_model.vision_model.encoder.layers)):
                if hasattr(self.base_model.vision_model.encoder.layers[layer].self_attn, 'instruction_out_proj') \
                        and self.base_model.vision_model.encoder.layers[layer].self_attn.instruction_out_proj is \
                        not None:
                    self.base_model.vision_model.encoder.layers[layer].self_attn.instruction_out_proj.load_state_dict(
                        self.base_model.vision_model.encoder.layers[layer].self_attn.out_proj.state_dict()
                    )
